...
int ei_main(int argc, char** argv)
{
	...
	ei_point_t	window_position		= {100, 10};

	ei_color_t white = {0xff,0xff,0xff,0xff};
	ei_color_t black = {0,0,0,0xff};

	ei_widget_t*	window2;
	ei_point_t	window2_position		= {200, 10};
    ...
    ei_size_t* min_size_toplevel = malloc(sizeof(ei_size_t));
    ei_size_t size_top = (ei_size_t){200, 200};
    min_size_toplevel = &size_top;
    ...
    ei_toplevel_widget_t* toplevel_test = (ei_toplevel_widget_t*) window;

    ei_widget_t* windowv2;
	windowv2 = ei_widget_create("toplevel", ei_app_root_widget());
	ei_point_t	window_positionv2		= {300, 210};
    min_size_toplevel = &size_top;
	ei_toplevel_configure(windowv2, &window_size, &window_color, &window_border_width,
				&window_title, &window_closable, &window_resizable, NULL);
	ei_place(windowv2, NULL, &(window_positionv2.x), &(window_positionv2.y), NULL, NULL, NULL, NULL, NULL, NULL);

//	/* Create, configure and place a button as a descendant of the toplevel window. */
	button = ei_widget_create("button", window);
    ei_size_t button_size = {1000, 100};
	...
	//ei_frame_configure	(button, &button_size, &button_color,
	//			 &button_border_width, &button_relief, &button_title, NULL, &button_text_color, NULL,
	//			NULL, NULL, NULL);
	ei_place(button, &button_anchor, &button_x, &button_y, NULL, NULL, &button_rel_x, &button_rel_y, &button_rel_width, NULL);

    ei_widget_t* buttonv2;
    int x=0, y=0;
    float rx=0.5, ry=0.5;
	char*		button_titlev2		= "Cancel";
	buttonv2 = ei_widget_create("button", window);
	ei_button_configure	(buttonv2, NULL, &button_color,
				 &button_border_width, NULL, &button_relief, &button_titlev2, NULL, &button_text_color, NULL,
				NULL, NULL, NULL, &button_callback, NULL);
	ei_place(buttonv2, NULL, NULL, NULL, NULL, NULL, &rx, &ry, NULL, NULL);
	/* Create, configure and place a toplevel window on screen */
	/*toplevel2 = ei_widget_create("toplevel", ei_app_root_widget());
	ei_toplevel_configure(toplevel2, &black, NULL, NULL, &window_title, &white, NULL, NULL);
=======


	/* Create, configure and place a toplevel window on screen */
	///toplevel2 = ei_widget_create("toplevel", window2);
	///ei_toplevel_configure(toplevel2, NULL, NULL, NULL, &window_title, NULL, NULL, NULL);
